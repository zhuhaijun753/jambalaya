...
//        PgSetAlpha(Pg_BLEND_SRC_1 | Pg_BLEND_DST_0, NULL, NULL, 0, 0);
        PgSetAlpha(Pg_ALPHA_OP_SRC_GLOBAL | Pg_BLEND_SRC_As | Pg_BLEND_DST_1mAs, NULL, NULL, 128, 0);
        PgAlphaOn();
    }
    ...
    for (it=0; it<2048; it++)
    {
        ...
        PgDrawILine(simple_random(1024*2), simple_random(600*2), simple_random(1024*2), simple_random(600*2));
    }
    PgFlush();
...
int create_window(int width, int height)
{
    ...
    PtSetArg(&winargs[winargc++], Pt_ARG_WINDOW_TITLE, "Line benchmark test", 0);
...
int main(int argc, char** argv)
{
    ...
    do {
        ...
        if (timeelapsed>5000)
        {
            clock_gettime(CLOCK_REALTIME, &starttime);
            fps=frames/5;
            fprintf(stdout, "FPS=%d, average lines per second=%d\n", fps, frames*2048/5);
            frames=0;
        }
